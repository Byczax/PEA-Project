       ...
       private static int GetLowerBound(Matrix matrix, int vertex)
        {
            ...
            {
                if (matrix.Array[i, vertex] < minIn)
                {
                    minIn = matrix.Array[i, vertex];
                }

                if (matrix.Array[vertex, i] < minOut)
                {
                    minOut = matrix.Array[vertex, i];
                }
            }
                ...
                else
                {
                    for (var i = 1; i < matrix.Size; i++)
                    {
                        ...
                        var childVertex = new BnBVertexPriority(matrix, i, vertex.BitMask | (1 << i), newRoad,
                            vertex.Vertices);
                        if (newRoad + vertex.LowerBound < upperBound)
                        {
